...
class _PagedSoftmax1(torch.autograd.Function):
    @staticmethod
    def forward(ctx, query, key_cache, value_cache, block_table, sequence_lengths, page_size, scale, is_causal, block_n):
        ...
        ctx.save_for_backward(query, key_cache, value_cache, block_table, sequence_lengths, row_max, row_denominator)
        ctx.args = (page_size, scale, is_causal, block_n)
        return output

    @staticmethod
    def backward(ctx, grad_output):
        query, key_cache, value_cache, block_table, sequence_lengths, row_max, row_denominator = ctx.saved_tensors
        page_size, scale, is_causal, block_n = ctx.args
        batch, query_heads, query_length, head_dim = query.shape
        kv_heads = key_cache.shape[1]
        max_key_length = int(sequence_lengths.max().item())
        block_d = _next_power_of_2(head_dim)
...
def paged_hopper_softmax1_attention(
    ...
    return _PagedSoftmax1.apply(
        query, key_cache, value_cache, block_table, sequence_lengths,
        page_size, actual_scale, is_causal, block_n,
    )
